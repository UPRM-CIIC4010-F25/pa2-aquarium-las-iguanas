...
void PowerUp::draw() const {
    ofLogVerbose() << "PowerUp at (" << m_x << ", " << m_y << ") with speed " << m_speed << std::endl;
    this->m_sprite->draw(this->m_x, this->m_y);
}
...
AquariumSpriteManager::AquariumSpriteManager(){
    ...
    this->m_power_up = std::make_shared<GameSprite>("power-up.png",70,70);
}
        ...
        if (event != nullptr && event->isCollisionEvent()) {
            ...
            if(event->creatureB != nullptr){
                ...
                if(this->m_player->getPower() < event->creatureB->getValue()){
                    ...
                    if(npcCreature->GetType()==AquariumCreatureType::PowerUp){
                        ofLogNotice() << "Power Up accquired " << "!" << std::endl;
                        this->m_player->changeSpeed(10);
                    }
...
void AquariumLevel::ConsumePopulation(AquariumCreatureType creatureType, int power){
    for(std::shared_ptr<AquariumLevelPopulationNode> node: this->m_levelPopulation){
        ...
        if(node->creatureType == creatureType){
            ...
            if(node->creatureType != AquariumCreatureType::PowerUp){ // Avoid respawning power up
